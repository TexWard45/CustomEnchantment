...
public class CEBook extends CEItem<CEBookData> {
	...
	public void importFrom(ItemStack source) {
		...
		CEEnchantSimple ceEnchantSimple = new CEEnchantSimple(tag.getString(CENBT.ENCHANT));

		CEBookData data = new CEBookData();
		data.setPattern(pattern);
		data.setCESimple(ceEnchantSimple);
		setData(data);
	}
	...
	public ApplyReason applyByMenuTo(CEItem ceItem) {
		...
		CEEnchantSimple ceEnchantSimple = this.getData().getCESimple();
		if (!enchant.isEnoughEnchantPointForNextCE(ceEnchantSimple)) {
			return new ApplyReason("not-enough-enchant-point", ApplyResult.CANCEL);
		}

		boolean canApplyMaterial = ceEnchantSimple.getCEEnchant().getAppliesMaterialList()
				.contains(new MaterialData(ceItem.getDefaultItemStack()));
		if (!canApplyMaterial) {
			return new ApplyReason("not-support-type", ApplyResult.CANCEL);
		}

		if (enchant.containsCESimple(ceEnchantSimple.getName())
				&& enchant.getCESimple(ceEnchantSimple.getName()).getLevel() >= ceEnchantSimple.getLevel()) {
			return new ApplyReason("conflict-enchant", ApplyResult.CANCEL);
		}

        if (enchant.hasEnchantBlacklist(ceEnchantSimple)) {
		...
		Chance success = new Chance(ceEnchantSimple.getSuccess().getValue());
		...
		if (success.work()) {
			...
			enchant.addCESimple(ceEnchantSimple);

			ceWeapon.updateTimeModifier();
			return reason;
		} else {
			Chance destroy = new Chance(ceEnchantSimple.getDestroy().getValue());
