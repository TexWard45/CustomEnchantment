...
public class CELoreFormat extends CEItem<CELoreFormatData> {

	public CELoreFormat(ItemStack itemStack) {
		super(CEItemType.LORE_FORMAT, itemStack);
	}
	...
	public void importFrom(ItemStack source) {
		...
		CELoreFormat item = (CELoreFormat) CustomEnchantment.instance().getCEItemStorageMap()
				.get(CEItemType.LORE_FORMAT).get(pattern);
	...
	public ItemStack exportTo(CELoreFormatData data) {
	...
	public Map<String, String> getPlaceholder(CELoreFormatData data) {
	...
	public ApplyReason applyByMenuTo(CEItem ceItem) {
        ...
        String currentWeaponSettingsType = ceWeapon.getWeaponSettingsName();
		if (currentWeaponSettingsType.equals(getData().getType())) {
			return new ApplyReason("same-type", ApplyResult.CANCEL);
		}

		ceWeapon.setWeaponSettingsName(getData().getType());
