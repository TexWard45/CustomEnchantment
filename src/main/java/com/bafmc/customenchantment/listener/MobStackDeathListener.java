	...
	public void onMobDeath(StackDeathEvent e) {
		...
		if (killer != null) {
			...
			if (!xpBonus.isEmpty()) {
				...
				ExpUtils.setTotalExperience(killer, ExpUtils.getTotalExperience(killer) + xp);
			}
			
			EntityTypeBonus mBonus = cePlayer.getMobBonus().getMoneyBonus();
			if (!mBonus.isEmpty()) {
				BafFrameworkAPI.giveMoneyLater(killer, mBonus.getBonus(entityType, e.getDeathStep()));
			}
