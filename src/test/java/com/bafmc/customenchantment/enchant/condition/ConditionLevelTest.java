...
/**
 * Tests for ConditionLevel - checks if player level matches comparison.
 */
@DisplayName("ConditionLevel Tests")
class ConditionLevelTest {
    ...
    class GetIdentifyTests {
        ...
        @Test
        @DisplayName("should return LEVEL")
        void shouldReturnCorrectIdentifier() {
            assertEquals("LEVEL", condition.getIdentify());
        }
    }
    ...
    class AllOperatorTests {
        ...
        @ParameterizedTest(name = "level={0}, op={1}, value={2} should be {3}")
        @CsvSource({
                "30, >, 10.0, true",
                "5, >, 10.0, false",
                "10, =, 10.0, true",
        ...
        @DisplayName("should compare player level with all operators")
        void shouldCompareLevel(int level, String operator, double value, boolean expected) {
    ...
    class EdgeCaseTests {
        ...
        @Test
        @DisplayName("should handle high level values")
        void shouldHandleHighLevelValues() {
            condition.setup(new String[]{">", "100.0"});

            Player mockPlayer = mock(Player.class);
            when(mockData.getPlayer()).thenReturn(mockPlayer);
            when(mockPlayer.getLevel()).thenReturn(500);
